# ── serez-apipack Test Runner ────────────────────────────────────────────────
# Usage: .\run_tests.ps1 [-sz <ruta>] [-filter <str>]
# CWD = serez-apipack/ → import "src/config" etc. resuelven localmente.
...
Write-Host "serez-apipack tests ($($tests.Count)) — sz: $sz`n" -ForegroundColor Cyan
$pass = 0; $fail = 0
